...
if(error==false){$('#contact-submit').attr({'disabled':'false','value':'در حال ارسال...'});$.post("sendmail.php",$("#contact-form").serialize(),function(result){if(result=='sent'){$('#cf-submit').remove();$('#mail-success').fadeIn(500);}else{$('#mail-fail').fadeIn(500);$('#contact-submit').removeAttr('disabled').attr('value','ارسال کردن پیام');}});}});});var wow=new WOW({offset:100,mobile:false});var scrolled=false;$(window).on('scroll',function(){if(!scrolled){scrolled=true;wow.init();}})
